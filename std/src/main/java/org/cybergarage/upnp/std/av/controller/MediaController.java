...
public class MediaController extends ControlPoint {
  ...
  private DeviceList getDeviceList(String deviceType) {
    ...
    for (int n = 0; n < allDevCnt; n++) {
      ...
      if (dev.isDeviceType(deviceType) == false) continue;
      devList.add(dev);
    }
    return devList;
  }
  ...
  public Device getServerDevice(String name) {
    ...
    if (foundDev.isDeviceType(MediaServer.DEVICE_TYPE)) return foundDev;
    return null;
  }

  public Device getRendererDevice(String name) {
    Device foundDev = getDevice(name);
    if (foundDev.isDeviceType(MediaRenderer.DEVICE_TYPE)) return foundDev;
    return null;
  }
  ...
  public Node browse(
      ...
      String sortCaiteria) {
    System.out.println(
        "browse " + objectID + ", " + browseFlag + ", " + startIndex + ", " + requestedCount);

    if (dev == null) return null;

    Service conDir = dev.getService(ContentDirectory.SERVICE_TYPE);
    if (conDir == null) return null;
    ...
    if (browseAction.postControlAction() == false) return null;
  ...
  public Node browseDirectChildren(
    ...
    return browse(
        dev,
        objectID,
        BrowseAction.BROWSE_DIRECT_CHILDREN,
        filter,
        startIndex,
        requestedCount,
        sortCaiteria);
  ...
  public ContainerNode browse(
      Device dev, String objectId, boolean hasBrowseChildNodes, boolean hasRootNodeMetadata) {
    ContainerNode contentRootNode = new RootNode();

    if (hasRootNodeMetadata) {
      Node rootNode = browseMetaData(dev, objectId, "*", 0, 0, "");
      if (rootNode != null) contentRootNode.set(rootNode);
    }
  ...
  private int browse(
      ContainerNode parentNode, Device dev, String objectID, boolean hasBrowseChildNodes) {
    ...
    for (int n = 0; n < nContents; n++) {
      ...
      } else if (ItemNode.isItemNode(xmlNode) == true) contentNode = new ItemNode();
  ...
  public void printContentNode(ContentNode node, int indentLevel) {
    ...
    for (n = 0; n < indentLevel; n++) System.out.print("  ");
  ...
  public void printMediaServers() {
    ...
    for (int n = 0; n < devCnt; n++) {
      ...
      if (dev.isDeviceType(MediaServer.DEVICE_TYPE) == false) continue;
      System.out.println(
          "["
              + n
              + "] "
  ...
  public boolean setAVTransportURI(Device dev, ItemNode itemNode) {
    ...
    if (resURL == null || resURL.length() <= 0) return false;
  ...
  public boolean play(Device dev, ItemNode itemNode) {
    stop(dev);
    if (setAVTransportURI(dev, itemNode) == false) return false;
    return play(dev);
  }

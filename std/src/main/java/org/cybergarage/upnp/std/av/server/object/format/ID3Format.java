...
public class ID3Format extends Header implements Format, FormatObject {
  ...
  public boolean loadHeader(InputStream inputStream) {
    try {
      ...
      for (int n = 0; n < HEADER_SIZE; n++) header[n] = dataIn.readByte();

      // Reading a extended header
      if (hasExtendedHeader() == true) {
        for (int n = 0; n < 4; n++) header[n] = dataIn.readByte();
        int extHeaderSize = getExtHeaderSize();
        // Ignoring extended header infos
        for (int n = 0; n < (extHeaderSize - 4); n++) dataIn.readByte();
      }

      // Reading frame infos
      frameList.clear();
      int frameDataSize = getHeaderSize() - HEADER_SIZE;
      if (hasExtendedHeader() == true) frameDataSize -= getExtHeaderSize();
      int frameDataCnt = 0;
      while (frameDataCnt < frameDataSize) {
        for (int n = 0; n < FRAME_HEADER_SIZE; n++) frameHeader[n] = dataIn.readByte();
        String frameID = getFrameID(frameHeader);
        int frameSize = getFrameSize(frameHeader);
        int frameFlag = getFrameFlag(frameHeader);
        byte frameData[] = new byte[frameSize];
        for (int i = 0; i < frameSize; i++) frameData[i] = dataIn.readByte();
  ...
  public int getHeaderSize() {
    int size = 0;
    for (int n = 0; n < 4; n++) size += (header[9 - n] & 0xFF) << n;
    return size;
  }
  ...
  public int getExtHeaderSize() {
    int size = 0;
    for (int n = 0; n < 4; n++) size += (extHeader[3 - n] & 0xFF) << n;
    return size;
  }
  ...
  private int getFrameSize(byte frameHeader[]) {
    int size = 0;
    for (int n = 0; n < 4; n++) size += (frameHeader[7 - n] & 0xFF) << n;
    return size;
  }
  ...
  public boolean equals(File file) {
    ...
    if (headerID.startsWith(HEADER_ID) == true) return true;
    return false;
  }
  ...
  public String getTitle() {
    ...
    if (0 < title.length()) return title;
  ...
  public String getCreator() {
    ...
    if (0 < creator.length()) return creator;

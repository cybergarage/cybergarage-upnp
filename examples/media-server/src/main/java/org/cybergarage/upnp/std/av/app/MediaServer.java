...
public class MediaServer extends org.cybergarage.upnp.std.av.server.MediaServer {
  ...
  /**** Mode Option ****/
  private static final int MODE_OPT_MASK = 0x00FF;

  private static final int FILESYS_MODE = 0x0000;
  private static final int MYTHTV_MODE = 0x0001;

  /**** Support Option ****/
  private static final int SUPPORT_OPT_MASK = 0xFF00;

  private static final int FLASH_SUPPORT = 0x0100;
  ...
  private void clearUserDirectoryPreferences() {
    try {
      ...
      for (int n = 0; n < dirCnt; n++) dirPref.remove(dirName[n]);
    } catch (Exception e) {
      Debug.warning(e);
    }
  }
  ...
  private void saveUserDirectories() {
    ...
    try {
      ...
      for (int n = 0; n < dirCnt; n++) {
        ...
        if (!(dir instanceof FileDirectory)) continue;
  ...
  public boolean stop() {
    if (getOption() == FILESYS_MODE) saveUserDirectories();
    return super.stop();
  }
  ...
  public static void main(String args[]) {
    ...
    for (int n = 0; n < args.length; n++) {
      ...
      if (MYTHTV_OPT_STRING.compareTo(args[n]) == 0) mode = MYTHTV_MODE;
      if (MYTHTV_OPT_STRING_OLD.compareTo(args[n]) == 0) mode = MYTHTV_MODE;
      if (VERBOSE_OPT_STRING.compareTo(args[n]) == 0) Debug.on();
      if (CONSOLE_OPT_STRING.compareTo(args[n]) == 0) need_gui = false;
    }

...
public class SwingFrame extends MediaFrame
  ...
  public SwingFrame(MediaServer server, boolean hasAddDelButtons) {
    ...
    DisplayMode mode =
        new DisplayMode(FRAME_WIDTH, FRAME_HEIGHT, 32, DisplayMode.REFRESH_RATE_UNKNOWN);
    ...
    dirPane
        .getScrollPane()
        .setPreferredSize(new Dimension(FRAME_WIDTH / 5, FRAME_HEIGHT /*-(FRAME_HEIGHT/10)*/));
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (srcObj == addButton) addDirectory();
    else if (srcObj == delButton) deleteDirectory();
    else if (srcObj == quitButton) exit();
  }
  ...
  private void addDirectory() {
    ...
    if (ret == JOptionPane.OK_OPTION) {
      ...
      if (dir == null) {
        JOptionPane.showMessageDialog(
            frame, "Please select a directory you want to add", TITLE, JOptionPane.WARNING_MESSAGE);
        return;
      }
      File dirFile = new File(dir);
      if (dirFile.exists() == false) {
        JOptionPane.showMessageDialog(
            frame, "Your selected directory is not found", TITLE, JOptionPane.WARNING_MESSAGE);
        return;
      }

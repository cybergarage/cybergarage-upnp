...
public class TvDevice
  ...
  private static final String CLOCK_DEVICE_TYPE = "urn:schemas-upnp-org:device:clock:1";
  private static final String CLOCK_SERVICE_TYPE = "urn:schemas-upnp-org:service:timer:1";

  private static final String LIGHT_DEVICE_TYPE = "urn:schemas-upnp-org:device:light:1";
  private static final String LIGHT_SERVICE_TYPE = "urn:schemas-upnp-org:service:power:1";

  private static final String AIRCON_DEVICE_TYPE = "urn:schemas-upnp-org:device:aircon:1";
  private static final String AIRCON_SERVICE_TYPE = "urn:schemas-upnp-org:service:temp:1";

  private static final String WASHER_DEVICE_TYPE = "urn:schemas-upnp-org:device:washer:1";
  private static final String WASHER_SERVICE_TYPE = "urn:schemas-upnp-org:service:state:1";
  ...
  public TvDevice() {
    ...
    tvDev.setSSDPBindAddress(HostInterface.getInetAddress(HostInterface.IPV4_BITMASK, null));
    tvDev.setHTTPBindAddress(HostInterface.getInetAddress(HostInterface.IPV4_BITMASK, null));
  ...
  public boolean isDevice(SSDPPacket packet, String deviceType) {
    ...
    if (usn.endsWith(deviceType)) return true;
    return false;
  }
  ...
  public void checkRemoveDevices(SSDPPacket packet) {
    if (isDevice(packet, CLOCK_DEVICE_TYPE) == true) clockTime = "";
    if (isDevice(packet, AIRCON_DEVICE_TYPE) == true) airconTemp = "";
  }
  ...
  public void deviceNotifyReceived(SSDPPacket packet) {
    if (packet.isAlive() == true) checkNewDevices(packet);
    if (packet.isByeBye() == true) checkRemoveDevices(packet);
  }
  ...
  public void eventNotifyReceived(String uuid, long seq, String name, String value) {
    ...
    if (service.isService(CLOCK_SERVICE_TYPE) == true) clockTime = value;
    else if (service.isService(AIRCON_SERVICE_TYPE) == true) airconTemp = value;
    else {

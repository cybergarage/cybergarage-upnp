...
public class UpnpIGDToolPane extends JPanel implements ActionListener {
  ...
  public Device getDevice(String devComboName) {
    if (devComboName == null) return null;

    DeviceList devList = ctrlPoint.getDeviceList();
    int devCnt = devList.size();
    for (int n = 0; n < devCnt; n++) {
      Device dev = devList.getDevice(n);
      if (devComboName.compareTo(getDeviceComboName(dev)) == 0) return dev;
    }
    return null;
  }
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (e.getSource() == addButton) {
      ...
      if (addPortDlg.doModal() == true) addPortMapping(addPortDlg);
